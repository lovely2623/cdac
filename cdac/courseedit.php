...
if (isset($_POST['update'])) {
    ...
    $updatedcourse = $conn->real_escape_string($_POST['course']);
 

    // Update SQL query
    $updateSql = "UPDATE courses SET Course = ? WHERE id = ?";
    $updateStmt = $conn->prepare($updateSql);
    $updateStmt->bind_param("si", $updatedcourse, $edit_id);
    $updateStmt->execute();
...
<div class="textboxx ">
    ...
    <div class="containeredit mt-5">
        <h2>Edit Course</h2>
        <form method="post" action="courseedit.php?id=<?php echo $edit_id;?>">
        <div class="form-group">
                <label>Course:</label>
                <input type="text" id="Course" name="course" value="<?php echo isset($row['Course']) ? htmlspecialchars($row['Course']) : ''; ?>" class="form-control">
            </div>
